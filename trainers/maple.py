...
class MaPLe(TrainerX):
    ...
    def _resolve_checkpoint_path(directory, registered_name, model_file):
        """Find checkpoint under registered or legacy subdirectory names."""
        candidates = [
            osp.join(directory, registered_name, model_file),
            osp.join(directory, "MaPLe_RouteC", model_file),
            osp.join(directory, "MultiModalPromptLearner", model_file),
    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            # Ignore fixed token vectors (prefixed or unprefixed)
            for k in [
                "token_prefix",
                "token_suffix",
            ...
            # Backward compat: prompt_learner-only checkpoint without prefix
            ...
            print('Loading weights to {} from "{}"{}'.format(name, model_path, msg_epoch))
